...
# assume setuptools certifi virtualenv installed in root install 202507.13MeV
...
for ver in /usr/bin/python3; do
  if [ -e $ver ]; then
    ...
    ${ver} -m pip install --upgrade \
     ...
     apache-libcloud apturl argcomplete asn1crypto astroid \
     Babel bcrypt blinker bracex Brlapi \
     cachetools certifi cffi chardet click colorama \
     command-not-found commonmark cryptography cupshelpers \
     defer distlib distro distro-info dnspython \
     enrich entrypoints \
     fasteners filelock flake8 folder-color future \
     ...
     keyring \
     language-selector launchpadlib lazr.restfulclient lazr.uri lazy-object-proxy \
     ...
     tenacity toml typed-ast typing-extensions \
     ubuntu-drivers-common ufw unattended-upgrades uritemplate urllib3 \
     vboxapi \
     wadllib wcmatch wheel wrapt \
     xattr xdg xkit xmltodict \
     yamllint \
     zipp
     #dbus-python \
     #python-apt \
